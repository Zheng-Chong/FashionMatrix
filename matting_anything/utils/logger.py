...
from matting_anything.utils import util
from matting_anything.utils.config import CONFIG
...
def normalize_image(image):
    ...
    return (image - image_flat.min(dim=1, keepdim=False)[0].view(3, 1, 1)) / (
            image_flat.max(dim=1, keepdim=False)[0].view(3, 1, 1) - image_flat.min(dim=1, keepdim=False)[0].view(3, 1,
                                                                                                                 1) + 1e-8)
